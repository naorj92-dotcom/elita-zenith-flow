...
export function TodaysFocusWidget() {
  const { data: focusItems = [] } = useQuery({
    ...
    queryFn: async (): Promise<FocusItem[]> => {
      ...
      // 1. Clients who completed today but didn't rebook
      ...
      if (completedToday && completedToday.length > 0) {
        ...
        if (notRebooked.length > 0) {
          items.push({
            id: 'no-rebook',
            icon: UserX,
            title: `${notRebooked.length} client${notRebooked.length > 1 ? 's' : ''} didn't rebook`,
            description: 'Follow up to maintain their treatment schedule',
            type: 'alert',
      ...
      if (now.getHours() < 17) {
        ...
        if ((remaining || 0) < 3) {
          items.push({
            id: 'open-slots',
            icon: Clock,
            title: 'Open slots this afternoon',
            description: 'Consider reaching out to waitlisted clients',
            type: 'insight',
      ...
      if (cancelledCount && cancelledCount > 0) {
        items.push({
          id: 'cancellations',
          icon: CalendarX,
          title: `${cancelledCount} cancellation${cancelledCount > 1 ? 's' : ''} today`,
          description: 'These slots can be filled from the waitlist',
          type: 'alert',
      ...
      // 4. Top service today
      const { data: todaysServices } = await supabase
        .from('appointments')
      ...
      if (todaysServices && todaysServices.length > 0) {
        ...
        if (topService) {
          items.push({
            id: 'top-service',
            icon: TrendingUp,
            title: `Top treatment: ${topService[0]}`,
            description: `${topService[1]} session${topService[1] > 1 ? 's' : ''} booked today`,
            type: 'insight',
          });
        }
      }
        ...
        <div className="space-y-3">
          {focusItems.map((item, i) => (
            <motion.div
              ...
              className={cn(
                'flex items-start gap-3 p-4 rounded-xl border-l-4 transition-all hover:shadow-sm',
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.actionLabel && item.actionHref && (
                  <Link to={item.actionHref}>
                    <Button variant="ghost" size="sm" className="h-7 text-xs px-0 mt-1.5 gap-1 text-primary hover:text-primary/80 hover:bg-transparent">
