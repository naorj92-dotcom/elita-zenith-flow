...
import {
  ...
  Settings2, Eye, Hammer, Hash, Mail, Phone, FileText, Heading,
  Minus, Image as ImageIcon, Camera, Star, Upload, ToggleLeft,
  Columns2, Columns3, RectangleHorizontal
} from 'lucide-react';
...
// Extended field type with width support
...
export function FormBuilderFull({ formData, onChange, onSave, onCancel, isSaving, isEditing, mode = 'form' }: FormBuilderFullProps) {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-background flex items-center justify-center">
        <div className="max-w-lg w-full px-6">
          <button type="button" onClick={onCancel} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-8 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back
          </button>
          <h1 className="text-2xl font-bold text-foreground mb-1">Create a {mode}</h1>
          <p className="text-muted-foreground mb-8">Pick a template or start blank</p>
            ...
            {tpls.map(t => (
              <button key={t.key} type="button" onClick={() => applyTemplate(t.key)}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{t.count} fields</Badge>
                </div>
              </button>
            ))}
            <button type="button" onClick={() => setShowTemplates(false)}
              className="w-full text-left border-2 border-dashed border-muted-foreground/20 rounded-xl p-5 hover:border-primary/30 transition-all">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Build your own {mode}</p>
                </div>
              </div>
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
      {/* ── Top Bar ── */}
      <div className="flex items-center justify-between border-b border-border px-4 h-14 shrink-0 bg-card">
        <div className="flex items-center gap-3">
          <button type="button" onClick={onCancel} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
          ...
          <div className="flex flex-col">
            <Input
              ...
              className="border-none shadow-none text-sm font-semibold h-6 px-1 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0"
            />
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={() => { set('is_active', true); onSave(); }} disabled={isSaving} className="gap-1.5">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left sidebar — Elements palette (build mode only) */}
        {view === 'build' && (
        ...
        {/* Right sidebar — Field properties (build mode + field selected) */}
        {view === 'build' && selectedField && selectedIdx !== null && (
...
/* ═══════ Build Canvas ═══════ */
function BuildCanvas({ fields, formData, selectedIdx, dragOverIdx, dragSourceIdx, setDragOverIdx, setDragSourceIdx, onSelect, onUpdate, onRemove, onDuplicate, onDrop, onAddField }: {
  ...
  return (
    <div className="max-w-[700px] mx-auto py-8 px-6">
      ...
      {formData.description && (
        <div className="bg-card rounded-lg p-4 mb-4 text-sm text-muted-foreground border border-border">
          {formData.description}
        </div>
      )}
      ...
      <div className="bg-card rounded-xl border border-border p-6 min-h-[420px]">
        {fields.length === 0 ? (
          <EmptyCanvas onDrop={onDrop} onAddField={onAddField} />
        ) : (
          <>
            {/* Drop zone at top */}
            ...
            <div className="mt-6 pt-4 border-t border-dashed border-border/50">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {FIELD_ELEMENTS.slice(0, 5).map(el => (
                  <button key={el.type} type="button" onClick={(e) => { e.stopPropagation(); onAddField(el.type); }}
                    className="flex items-center gap-1 px-2 py-1 rounded text-[10px] text-muted-foreground hover:text-foreground hover:bg-accent border border-transparent hover:border-border transition-all">
...
/* ═══════ Field Card ═══════ */
function FieldCard({ field, index, isSelected, isDragSource, onSelect, onUpdate, onRemove, onDuplicate, setDragSourceIdx }: {
  ...
  return (
    <div
      ...
      className={cn(
        'relative group rounded-lg transition-all py-2.5 px-3 my-0.5',
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            {field.width && field.width !== 'full' && (
              <Badge variant="outline" className="text-[9px] h-4 px-1 border-muted-foreground/20 text-muted-foreground">
                {field.width === 'half' ? '½' : '⅓'}
              </Badge>
            )}
          </div>
...
function FieldVisual({ field }: { field: ExtendedFormField }) {
  ...
  switch (field.type) {
    ...
    case 'textarea':
      return <div className={cn(base, 'h-16 flex items-start p-2')}><span className="text-[10px] text-muted-foreground/50">{field.placeholder || 'Enter text...'}</span></div>;
    case 'date':
...
/* ═══════ Empty Canvas ═══════ */
function EmptyCanvas({ onDrop, onAddField }: { onDrop: (e: React.DragEvent, idx: number) => void; onAddField: (t: string) => void }) {
  const [hovering, setHovering] = useState(false);
  return (
    <div
      className={cn('py-16 text-center rounded-lg transition-all border-2 border-dashed',
    ...
    >
      <div className="w-14 h-14 rounded-xl bg-muted mx-auto mb-4 flex items-center justify-center">
        <Plus className="w-7 h-7 text-muted-foreground" />
      </div>
      <p className="text-sm font-semibold text-foreground mb-1">Add your first field</p>
      <p className="text-xs text-muted-foreground mb-5">Drag from the sidebar or click below</p>
      <div className="flex items-center justify-center gap-2 flex-wrap px-4">
        {FIELD_ELEMENTS.slice(0, 4).map(el => (
    ...
    <div className="max-w-[700px] mx-auto py-8 px-6">
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="text-center py-8 px-6">
          ...
          {formData.description && <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto">{formData.description}</p>}
        </div>
        <Separator />
        ...
        <div className="p-6 pt-0"><Button className="w-full h-11" disabled>Submit Form</Button></div>
      </div>
    </div>
  );
}
