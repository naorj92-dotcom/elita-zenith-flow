      ...
      <div className="p-6 lg:p-8 space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-muted/50 p-1">
            ...
            <TabsTrigger value="timeclock" className="data-[state=active]:bg-background">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-center py-3 border-b border-border">
                    ...
                    <span className="font-medium text-success">
                      {formatCurrency(transactions.filter(t => t.transaction_type === 'service').reduce((sum, t) => sum + (Number(t.commission_amount) || 0), 0))}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-3 border-b border-border">
                    <span className="text-muted-foreground">Retail Commission</span>
                    <span className="font-medium text-success">
                      {formatCurrency(transactions.filter(t => t.transaction_type === 'retail').reduce((sum, t) => sum + (Number(t.commission_amount) || 0), 0))}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-3 bg-primary/5 rounded-lg px-3">
                    <span className="font-semibold">Total Earnings</span>
                    <span className="font-bold text-primary text-lg">{formatCurrency(totalEarnings)}</span>
                  </div>
                </CardContent>
              </Card>
