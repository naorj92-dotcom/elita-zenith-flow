...
import { Shield, Search, Filter, Clock, User, AlertTriangle, LogIn, LogOut, XCircle, Monitor } from 'lucide-react';
...
const SECURITY_EVENT_COLORS: Record<string, string> = {
  login_success: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400',
  login_failed: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400',
  logout: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400',
};

const SECURITY_EVENT_ICONS: Record<string, React.ReactNode> = {
  login_success: <LogIn className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />,
  login_failed: <XCircle className="w-4 h-4 text-red-600 dark:text-red-400" />,
  logout: <LogOut className="w-4 h-4 text-blue-600 dark:text-blue-400" />,
};
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">
            Security Events ({securityLogs.length} entries)
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                return (
                  ...
                  >
                    <div className="mt-0.5">
                      {SECURITY_EVENT_ICONS[log.event_type] || <Shield className="w-4 h-4 text-muted-foreground" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <Badge
                          variant="secondary"
                          className={SECURITY_EVENT_COLORS[log.event_type] || 'bg-muted text-muted-foreground'}
                        >
                          {log.event_type?.replace(/_/g, ' ')}
                        </Badge>
                        {meta?.email && (
                          <span className="text-sm text-foreground font-medium">{meta.email}</span>
                        )}
                        {meta?.error && (
                          <span className="text-xs text-destructive italic">— {meta.error}</span>
                        )}
                      </div>
